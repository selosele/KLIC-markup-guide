...
- [1. 공통 규칙](#1-공통-규칙)
  ...
  - [1.2. 네이밍](#12-네이밍)
    ...
    - [1.2.6. 클래스 네이밍 확장](#126-클래스-네이밍-확장)
...
- [3. CSS 코드 작성 규칙](#3-css-코드-작성-규칙)
  ...
  - [3.12. 특정 class 사용 금지](#312-특정-class-사용-금지)
- [참고 사이트](#참고-사이트)
...
CSS 파일의 작업자 정보는 최상단 CSS 인코딩 선언과 ```@import``` 구문 다음 줄에 표기한다.
...
```.MC_wrap```과 ```.MC_box```는 개수에 제한이 없고, 한 개밖에 없어도 숫자 1을 표기하며, 디자인 상 ```.MC_box```를 감쌀 필요 없을 경우 ```.MC_wrap```을 생략할 수 있다.
...
동일한 ID는 한 파일에서 한 번만 사용할 수 있으며, 레이아웃과 관련된 요소 또는 JS 작성을 위해서만 사용한다.
