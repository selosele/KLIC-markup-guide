...
웹 브라우저는 웹 페이지를 구성하는 텍스트 콘텐츠의 언어 정보를 바탕으로 텍스트 콘텐츠를 화면에 표시하거나 보조 기술로 전달하므로, 웹 페이지의 기본 언어를 정확히 정의해야 한다. **html 태그의 lang 속성에 ISO639-1에서 지정한 언어 코드를 명시해주도록 한다.** 많이 사용되는 코드로는 한국어 사이트의 경우 ko, 영문 en, 일문 ja 등이 있다.
