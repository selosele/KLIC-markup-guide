...
- [2. HTML 코드 작성 규칙](#2-html-코드-작성-규칙)
  ...
  - [2.7. 순서에 맞는 Heading 태그 사용](#27-순서에-맞는-heading-태그-사용)
- [3. CSS 코드 작성 규칙](#3-css-코드-작성-규칙)
  ...
  - [3.7. 단위 변환](#37-단위-변환)
  - [3.8. 통일된 공통 클래스 사용](#38-통일된-공통-클래스-사용)
  - [3.9. 태그 선택자 사용 지양](#39-태그-선택자-사용-지양)
...
본 문서는 케이엘정보통신㈜의 웹 퍼블리싱 작업자가 소스 코드 작성 시에 따라야 할 규칙을 기술하며, 프로젝트와 유지보수에 투자되는 시간을 최소화하기 위한 통일된 코드 작성법을 제시한다. 웹 퍼블리싱 작업자는 본 문서의 규칙을 반드시 지켜야 한다.
...
**잘못된 예 : div는 section 등으로 바꿀 수 있고, span은 p/div/a 등으로 바뀔 수 있다.**
...
**올바른 예 : 태그명이 바뀌어도 틀어지지 않아 2차 수정이 발생하지 않는다.**
...
최소값 1, 최대값 1000이고 5 단위로 증감하며, ```-1```과 기본값인 ```auto```는 예외적으로 사용할 수 있다. 단, 5단위 사이의 예외 변수가 발생하면 1단위의 값을 지정할 수 있다. 레이어 팝업처럼 시각적으로 제일 위에 보여야 하는 경우 최대값을 지정한다.
