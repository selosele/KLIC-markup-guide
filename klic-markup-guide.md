...
* [2. HTML 코드 작성 규칙](#user-content-2-html-코드-작성-규칙)
    ...
    * [2.5. ID값 선언](#user-content-25-id값-선언)
        * [2.5.1. ID값 표기법](#user-content-251-id값-표기법)
    * [2.6. 속성 선언 시 참고](#user-content-26-속성-선언-시-참고)
...
본 문서가 최신의 내용으로 유지되도록 관리하는 책임은 작성담당자에게 있고, 내용의 정정/개선 요구를 작성담당자에게 요청할 수 있으며, 담당자는 요청 내용을 검토하여 본 문서를 개정할 수 있다.
...
다른 이가 작성한 파일을 타 프로젝트에 복사 후 수정이 발생한 경우 마지막 단락에 Editor를 추가, 작업자명과 날짜를 표기하도록 한다.
...
이미지 파일명은 **형태_의미_상태** 순서로 표기하며, 숫자로 시작할 수 없고 영문 대문자를 사용하지 않는다.
...
웹 브라우저는 웹 페이지를 구성하는 텍스트 콘텐츠의 언어 정보를 바탕으로 텍스트 콘텐츠를 화면에 표시하거나 보조 기술로 전달하므로, 웹 페이지의 기본 언어를 정확히 정의해야 한다. **html 태그의 lang 속성에 ISO639-1에서 지정한 언어 코드를 표기해주도록 한다.** 많이 사용되는 코드로는 한국어 사이트의 경우 ko, 영문 en, 일문 ja 등이 있다.

```
<html lang="en">
```

위와 같이 한국어 사이트임에도 영어에 해당하는 언어 코드가 표기된 것은 잘못된 예시이다.
...
class 속성은 태그의 필수 속성들 마지막에 선언해주고, id 속성을 제일 마지막에 선언한다. 그리고 style 속성이 불가피하게 들어갈 경우 id 속성의 다음 제일 마지막에 선언해준다.  
예외사항 제외 속성의 정해진 순서는 없으나 가급적이면 추후 일괄수정하기 편하도록 순서를 지켜서 작성한다.
